...
class ExternalShuffleService(sparkConf: SparkConf, securityManager: SecurityManager)
  ...
  protected def newShuffleBlockHandler(conf: TransportConf): ExternalBlockHandler = {
    ...
    // path containing the app id). The check is opt-in on this branch: executors launched
    // by Workers that predate the per-app layout register unscoped paths and would be
    // rejected, so enable it only once every Worker is upgraded.
    ...
    if (sparkConf.get(config.SHUFFLE_SERVICE_DB_ENABLED) && enabled) {
      ...
      new ExternalBlockHandler(conf,
        findRegisteredExecutorsDBFile(dbBackend.fileName(registeredExecutorsDB)), localDirs,
