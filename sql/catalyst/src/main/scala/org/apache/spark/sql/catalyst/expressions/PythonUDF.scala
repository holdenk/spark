...
case class TranspiledPythonUDF(
  ...
  // Per-option input-type categories (one per public param, drawn from
  // "numeric"/"integral"/"integral32"/"fractional"/"string"/"bool"/"binary" -- see
  // ResolveTranspiledPythonUDFOptions for what each matches and why the numeric ones
  // overlap), parallel to `transpiledOptions`. ResolveTranspiledPythonUDFOptions prunes the
  // options to those whose categories match the resolved input types and that themselves
  // resolve (before CheckAnalysis can reject a type-incompatible or unresolved option) and
  // clears this field; ConvertToCatalyst then picks the first survivor or falls back to the
  // Python UDF. Empty means "no restriction" (kept as-is).
